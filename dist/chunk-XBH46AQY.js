...
var worktreeMutationLock = Effect_exports.unsafeMakeSemaphore(1);
...
var unlockDeadWorktrees = (repoDir) => Effect_exports.gen(function* () {
  const fs = yield* FileSystem_exports.FileSystem;
  const list = yield* execGit(["worktree", "list", "--porcelain"], repoDir);
  for (const record2 of list.split("\n\n")) {
    const lines2 = record2.split("\n");
    const path = lines2.find((line) => line.startsWith("worktree "))?.slice("worktree ".length).trim();
    if (!path) continue;
    if (!lines2.some((line) => line === "locked" || line.startsWith("locked ")))
      continue;
    const stillThere = yield* fs.exists(path).pipe(Effect_exports.catchAll(() => Effect_exports.succeed(true)));
...
//# sourceMappingURL=chunk-XBH46AQY.js.map
//# sourceMappingURL=chunk-XBH46AQY.js.map